...
import { TrainingRegistrationDeleteButton } from '@/components/admin/training-registration-delete-button'
...
export default async function AdminTrainingsPage({
  ...
  const activeTab = params?.tab === 'training-events' ? 'training-events' : 'registration-requests'
      ...
      <div>
        <div className="mb-6">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Manage the public training schedule and submitted registration requests.
          </p>
        </div>

        <Tabs defaultValue={activeTab} className="space-y-6">
          <TabsList className="h-auto flex-wrap justify-start rounded-2xl p-1">
            <TabsTrigger asChild value="training-events" className="rounded-xl px-4 py-2">
              <Link href="/admin/trainings?tab=training-events">
                Training Events
                <Badge variant="secondary" className="ml-1 rounded-full">{events.length}</Badge>
              </Link>
            </TabsTrigger>
            <TabsTrigger asChild value="registration-requests" className="rounded-xl px-4 py-2">
              <Link href="/admin/trainings?tab=registration-requests">
                Registration Requests
                <Badge variant="secondary" className="ml-1 rounded-full">{registrations.length}</Badge>
              </Link>
            </TabsTrigger>
          </TabsList>

          <TabsContent value="training-events" className="space-y-5">
            <TrainingEventForm />
            ...
            <section>
              <div className="mb-6">
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Incoming training and webinar submissions.
                </p>
              </div>
                ...
                <div className="space-y-4">
                  ...
                  <div key={String(r.id)} className="rounded-2xl border bg-card p-5">
                    ...
                    <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                      <p className="text-xs text-muted-foreground">{formatDate(String(r.created_at))}</p>
                      <TrainingRegistrationDeleteButton
                        registrationId={String(r.id)}
                        registrantName={String(r.full_name ?? 'this registrant')}
                      />
                    </div>
                  </div>
                  ))}
