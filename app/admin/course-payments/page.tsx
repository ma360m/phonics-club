...
import { ChevronDown, ExternalLink, FileSearch, KeyRound, Mail, ShieldCheck, XCircle } from 'lucide-react'
...
function CoursePaymentCard({ payment }: { payment: any }) {
  ...
  const paymentType = certificatePayment ? 'Certificate payment' : 'Course access payment'

  return (
    <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
      <details className="group">
        <summary className="grid cursor-pointer list-none gap-4 md:grid-cols-[minmax(0,1.25fr)_minmax(0,1fr)_auto] md:items-center [&::-webkit-details-marker]:hidden">
          <div className="min-w-0">
            ...
            <h2 className="mt-3 truncate text-lg font-bold text-[#0F172A]">{payment.courses?.title ?? 'Course payment'}</h2>
            <p className="mt-1 text-xs font-semibold uppercase tracking-wide text-[#1D4ED8]">{paymentType}</p>
            <p className="mt-2 truncate text-sm font-semibold text-[#0F172A]">Student: {studentName}</p>
            <p className="truncate text-xs text-slate-500">Email: {studentEmail || 'No email on profile'}</p>
          </div>

          <div className="grid gap-2 text-sm text-slate-600 sm:grid-cols-3 md:grid-cols-1 xl:grid-cols-3">
            <div>
              <p className="text-[11px] font-bold uppercase tracking-wide text-slate-500">Workflow</p>
              <p className="mt-1 font-semibold text-[#0F172A]">{workflowLabel(workflowStatus)}</p>
            </div>
            <div>
              <p className="text-[11px] font-bold uppercase tracking-wide text-slate-500">Receipt</p>
              <p className="mt-1 font-semibold text-[#0F172A]">{slipUploaded ? 'Uploaded' : 'Not uploaded'}</p>
            </div>
            <div>
              <p className="text-[11px] font-bold uppercase tracking-wide text-slate-500">Licence</p>
              <p className="mt-1 font-semibold text-[#0F172A]">{certificatePayment ? 'Not needed' : licenceIssued ? 'Issued' : 'Not issued'}</p>
            </div>
          </div>

          <div className="flex flex-col gap-2 md:items-end">
            <p className="text-2xl font-bold text-[#1D4ED8]">{formatPrice(Number(payment.amount ?? 0), payment.currency ?? 'PKR')}</p>
            <span className="inline-flex h-9 items-center justify-center gap-2 rounded-lg border border-slate-200 bg-white px-3 text-sm font-semibold text-[#1D4ED8] shadow-sm transition-colors group-open:border-[#BFDBFE] group-open:bg-[#EFF6FF]">
              Payment details
