...
const fastUpdateRowSchema = z.object({
  ...
  stock_management_enabled: z.boolean(),
  backorder_policy: z.enum(['disabled', 'enabled', 'enabled_with_warning']),
  max_backorder_quantity: z.coerce.number().int().min(0).nullable(),
...
export async function fastUpdateProductsAction(
  ...
  for (const row of parsed.data) {
    const payload: Record<string, unknown> = {
      ...
      stock_management_enabled: row.stock_management_enabled,
      backorder_policy: row.backorder_policy,
